...
export class StatusEventController {
  ...
  async handleStatus(@Payload() data: StatusData, @Ctx() context: RmqContext) {
    ...
    try {
      ...
      this.logger.error(
        'Failed to ack RMQ message:',
        (ackErr as Error).stack ?? ackErr,
      );
    }
  }
  ...
  private async processGoWorkerStatus(data: StatusData) {
    const {
      ...
      errorMsg: errorMsgRaw,
    } = data;

    this.logger.log(
      `Processing Go worker status - id:${id}, user:${userId}, status:${statusRaw}`,
    );

    const status =
      statusRaw === STATUS.PROCESSING
        ? STATUS.PROCESSING
        : statusRaw === STATUS.PROCESSED
          ? STATUS.PROCESSED
          : STATUS.FAILED;

    const publicUrl =
      publicUrlRaw === undefined || publicUrlRaw === '' ? null : publicUrlRaw;
    const errorMessage =
      errorMsgRaw === undefined || errorMsgRaw === '' ? null : errorMsgRaw;
    ...
    try {
      ...
      this.logger.log(
        `Updated DB record ${updatedData.id} (user ${updatedData.userId}) - status ${status}`,
      );
    } catch (dbErr) {
      this.logger.error(
        'DB update failed for status message:',
        (dbErr as Error).stack ?? dbErr,
      );
    }
  }
}
