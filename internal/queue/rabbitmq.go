...
func (rabbitMqService *RabbitMqService) declareExchange() error {
	if rabbitMqService.statusQueueChannel == nil {
		return errors.New("statusQueueChannel is nil, cannot declare exchange")
	}
...
func (rabbitMqService *RabbitMqService) PublishToChannel(ctx context.Context, message interface{}) error {
	if rabbitMqService.statusQueueChannel == nil {
		return errors.New("statusQueueChannel is not initialized")
	}
...
func (rabbitMqService *RabbitMqService) transformImage(imageProcessing types.ImageProcessing) error {
	...
	case "RESIZE":
		...
		formattedUploadPath, err := utils.PathUtil(path.Join(uploadPath, "processed"), parts[1])
...
func (rabbitMqService *RabbitMqService) ProcessMessage(ctx context.Context, d amqp.Delivery) error {
	...
	if downloadErr != nil {
		...
		return ProcessingError{Err: fmt.Errorf("download failed for key %s: %w", rabbitMqMessage.Data.S3RawKey, downloadErr), Requeue: false}
	}

	log.Printf("Successfully processed object: %s", rabbitMqMessage.Data.S3ProcessedKey)

	if err := rabbitMqService.transformImage(rabbitMqMessage.Data); err != nil {
		log.Printf("Transform failed: %v", err)
		errorMsg = ErrTransform
		status := types.FAILED
		statusData := utils.InitStatusData(rabbitMqMessage.Data.Id, rabbitMqMessage.Data.UserId, status, "", errorMsg)
		statusMessage := utils.InitStatusMessage(statusData)
		if publishErr := rabbitMqService.PublishToChannel(ctx, statusMessage); publishErr != nil {
		...
		return ProcessingError{Err: fmt.Errorf("transform failed for key %s: %w", rabbitMqMessage.Data.S3RawKey, err), Requeue: false}
	}
	...
	if len(splitString) < 2 {
		...
		return ProcessingError{Err: fmt.Errorf("unexpected S3RawKey format: %s", rabbitMqMessage.Data.S3RawKey), Requeue: false}
	}
	...
	if uploadErr != nil {
		...
		return ProcessingError{Err: fmt.Errorf("upload failed for key %s: %w", formattedKey, uploadErr), Requeue: false}
	}
